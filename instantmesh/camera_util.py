...
def get_optimize_cameras(batch_size=1, radius=4.0):
    """
    Get the input camera parameters.
    """
    azimuths = np.array([30, 90, 150, 210, 270, 330]).astype(float)
    ...
    return cameras.unsqueeze(0).repeat(batch_size, 1, 1, 1)
def get_optimize_cameras_GT(batch_size=1, radius=4.0):
    """
    Get the input camera parameters.
    """
    azimuths = np.array([0, 30, 90, 150, 210, 270, 330]).astype(float)
    elevations = np.array([20, 20, -10, 20, -10, 20, -10]).astype(float)
